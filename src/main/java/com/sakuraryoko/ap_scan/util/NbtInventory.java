...
public class NbtInventory implements AutoCloseable
{
     ...
     * @return ()
     */
    public static @Nullable NbtInventory fromVanillaList(@Nonnull NonNullList<ItemStack> list)
    {
        int size = list.size();
     ...
     */
    public Tag toNbtSingle(@Nonnull RegistryAccess registry) throws RuntimeException
    {
        if (this.size() > 1)
        {
...
//            LOGGER.info("toNbtSingle(): --> nbt: [{}]", element.toString());
    ...
    public static @Nullable NbtInventory fromNbtList(@Nonnull ListTag list, boolean noSlotId, @Nonnull RegistryAccess registry) throws RuntimeException
    {
        ...
        for (int i = 0; i < list.size(); i++)
        {
			...
			CompoundTag nbt = checkForIDOverrides((CompoundTag) list.get(i));
            ...
            if (noSlotId)
            {
                slot = new ItemStackWithSlot(i, ItemStack.CODEC.parse(registry.createSerializationContext(NbtOps.INSTANCE), nbt).getPartialOrThrow());
            }
            else
            {
                slot = ItemStackWithSlot.CODEC.parse(registry.createSerializationContext(NbtOps.INSTANCE), nbt).getPartialOrThrow();
            }
...
//        newInv.dumpInv();
	...
	private static CompoundTag checkForIDOverrides(CompoundTag in)
	{
