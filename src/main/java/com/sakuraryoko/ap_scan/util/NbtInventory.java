...
//                        Tag element = ItemStackWithSlot.CODEC.encodeStart(registry.createSerializationContext(NbtOps.INSTANCE), slot).getPartialOrThrow();
    ...
    /**
     * Creates a new {@link NbtInventory} from a single-member {@link CompoundTag} containing a single item with a slot number.
     *
     * @param tag -
     * @return -
     * @throws RuntimeException -
...
//        newInv.dumpInv();
    ...
    public static class EntrySlot
    {
        ...
        public CompoundTag toNbt(@Nonnull RegistryAccess registry)
        {
            ...
            {
                LOGGER.error("EntrySlot#toNbt: Exception Serializing Item: [{}]; {}", this.stack.getItemName().getString(), e.getLocalizedMessage());
                final String id = this.stack.getItemName().getString();
                final Component text = id.isEmpty()
                                       ? Component.nullToEmpty("§cUnparsable/Invalid Item§r")
                                       : Component.nullToEmpty("§cUnparsable/Invalid Item: '"+id+"'§r");
        ...
        public static EntrySlot fromNbt(CompoundTag nbt, @Nonnull RegistryAccess registry)
        {
            ...
            {
                LOGGER.error("EntrySlot#fromNbt: Exception Deserializing Item: [{}]; {}", nbt.toString(), e.getLocalizedMessage());
